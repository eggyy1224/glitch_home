...
- `src/App.jsx`：解析 URL 參數 → 決定 DisplayMode → 注入對應元件，並整合 `useScreenshotManager`、`useControlSocket`、`useRemoteTimelineControl`、`useCameraPresets` 等 hooks。這裡同時建立 `createModeRenderMap()`、時間軸覆蓋層、音效佇列與字幕 overlay。
- `src/components/`：共用 UI（ControlPanel、ModeLayout、ScreenshotMessage、SubtitleCaptionStatus）、Kinship 場景（`components/kinship/**`）、搜尋面板與 `IframeTimelineControls`（顯示遠端時間軸狀態）。
- `src/modes/`：目前僅有 `createModeRenderMap.js`，集中管理各模式的 component 與 `onCaptureReady` 註冊規則，避免在 `App.jsx` 寫長串的 `switch`。
- `src/hooks/`：包含模式判斷 (`useModeParams`)、WebSocket (`useControlSocket` + `useControlSocketHandlers`)、截圖 (`useScreenshotManager`)、字幕 (`useSubtitleCaption`)、iframe/collage 設定、相機預設、幻燈片播放、時間軸播放器 (`useIframeTimelinePlayer`) 與步驟動作 (`useTimelineStepActions`)。
- `src/utils/`：iframe/collage 設定解析、collage 切片數學、slide 模式工具、html2canvas loader 等純函式；`src/constants/` 放置 collage 預設常數。
- `src/styles/` 與各模式 `.css` 控制視覺，以及 `styles/IframeTimelineControls.css` 處理時間軸控制面板。
- `src/api.js`：集中所有 REST 呼叫，除了既有的親緣/搜尋/截圖/拼貼/生成，也包含字幕、語音、時間軸、remote click／video control 等最新 API。
...
- **IFRAME**：`IframeMode.jsx`。讀取 `useIframeConfig` 產出的配置，支援 grid/horizontal/vertical 佈局、面板拖拉調整比率以及本地設定面板；可從 WebSocket 收到遠端更新與 snapshot 還原，若 `useRemoteTimelineControl` 啟動還會顯示 `IframeTimelineControls` 並自動套用 snapshot。
...
- **STATIC**：`StaticMode.jsx`。單張全螢幕靜態圖，支援 `object_fit`、`object_position` 參數，並可提供截圖給後端。
  - URL 可加 `img_base=/nightwalk_assets/` 來覆寫圖片來源（長期對應 `夜遊 - 毛刺` 目錄）。
- **VIDEO**：`VideoMode.jsx`。播放 `/videos/圖像系譜學Video/<file>`，供截圖，並透過 `videoControllerRef` 支援遠端播放/暫停/mute/seek 指令。

所有模式都包在 `ModeLayout.jsx`：該元件會佈局 before/after content、注入 `onCaptureReady`、掛上 `SoundPlayer` 與 `SubtitleOverlay`；音效播放請求由 `useSoundQueue` 來回傳遞。
...
### 4.1 `useModeParams`
- 解析 `window.location.search`，讀出 `img`、`slide_interval`、`sound_player`、`iframe_timeline`、`client` 等。
- 透過 `useDisplayMode` 決定實際 Mode，並輸出 `incubatorMode` / `phylogenyMode` 與 `shouldLoadKinshipData`（iframe/slide/organic/static/video 模式會直接略過親緣 API）。
- `clientId` 來源：`?client=`、環境變數 `VITE_CLIENT_ID`、預設 `default`；同時回傳 `iframeTimelineId`，供遠端時間軸預載使用。
...
- `createModeRenderMap` 會把所有模式的 component、`withCaptureReady` 設定與 before/after content 統一回傳，讓 `App.jsx` 只需依 display mode 取對應元素。

### 4.3 截圖／WebSocket／音效
- `useControlSocket` 打 `/ws/screenshots`，自動送 `hello` (client id)、簡易 2 秒重連。
- `useControlSocketHandlers` 統一篩選 `target_client_id`，並處理 `subtitle_update`、`caption_update`、`iframe_config`、`collage_config`、`unlock_audio`（觸發隱藏靜音 audio 以解鎖 autoplay）、`remote_click`、`video_control`、`timeline_control` 等事件。
...
- `useSoundQueue` / `SoundPlayer` 負責把 `sound_play` 事件轉成音效清單，必要時會自動向 `/api/sound-files` 重新取檔案並播放。
...
### 4.8 遠端時間軸與自動化
- `useRemoteTimelineControl` 會檢查 URL `iframe_timeline` 或 WS `timeline_control` 事件，決定是否強制切換至 iframe 模式並播放指定 timeline；亦提供 `play/pause/stop/next/previous/reload` 介面給 `IframeTimelineControls`。
- `useIframeTimelinePlayer` 在 iframe 模式啟用時，拉取 `/api/iframe-timelines/{id}`，依 step duration 排程切換 snapshot，並透過 `applyRemoteIframeConfig` 套用面板設定；若 timeline loop 停止或切換模式，會自動釋放遠端配置。
- `useTimelineStepActions` 在每個步驟開始時執行後端動作（`setSubtitle`/`clearSubtitle`、`setCaption`/`clearCaption`、`triggerTts`、`speakWithSubtitle`、`queueSoundPlay`、`sendRemoteClick`、`sendVideoControl`、`unlockAudio` 等），並提供錯誤回報給 UI。
- `videoControllerRef` 傳給 `VideoMode` 以支援 timeline 內的 video 控制指令；`unlockAudioElementRef` 則讓 `handleUnlockAudioMessage` 能在無 user gesture 下觸發一次靜音播放。
...
- **WebSocket 與廣播**：`useControlSocket` 維持與 `/ws/screenshots` 的單一連線，集中處理 `screenshot_request/completed`、`sound_play`、`subtitle_update`、`caption_update`、`iframe_config`、`collage_config`、`unlock_audio`、`remote_click`、`video_control`、`timeline_control` 等事件，再分派給對應 hook。
...
- **親緣／視角／時間軸**：`fetchKinship`、`fetchCameraPresets`、`saveCameraPreset`、`deleteCameraPreset`、`fetchIframeTimeline`（載入 backend metadata/timeline 定義）。
- **搜尋與分析**：`searchImagesByImage`、`searchImagesByText`；以圖搜圖時會先透過 `/api/screenshots` 上傳檔案取得路徑再重查。
- **截圖佇列**：`uploadScreenshot`、`reportScreenshotFailure` 讓前端能回報自動截圖成功或失敗。
- **拼貼與配置**：`fetchCollageConfig`、`saveCollageConfig`、`generateCollageVersion`（multipart 上傳）、`generateCollageVersionFromNames`、`getCollageProgress`。
- **生成**：`generateMixTwo`（串 `/api/generate/mix-two`）、`listOffspringImages`。
- **字幕／音效／遠端控制**：`fetchSubtitleState`、`fetchCaptionState`、`fetchSoundFiles`、`triggerTts`、`speakWithSubtitle`、`queueSoundPlay`、`setSubtitle`、`clearSubtitle`、`setCaption`、`clearCaption`、`unlockAudio`、`sendRemoteClick`、`sendVideoControl`。SoundPlayer 會把 `/api/sound-files` 的 URL 轉成 encode 後的絕對路徑。
...
- `IframeTimelineControls`：當遠端 timeline 啟用時會出現在畫面上方，可直接播放/暫停/停止/上一段/下一段/重新載入。
- `VideoMode` / `StaticMode` / `IframeMode`：點擊即可準備截圖函式；Video 支援點擊/Enter/Space 切換靜音，並能透過 timeline／`video_control` 指令被遠端操作。
...
- 遠端時間軸 JSON 尚無前端編輯工具，需直接修改 `backend/metadata/timelines/iframe/*.json`；修改後須重新整理前端才會載入最新定義。
