...
│   ├── realtime.py      # subtitle/caption、screenshot request、WebSocket、remote control
│   └── timeline.py      # iframe 時間軸列表/播放/停止，廣播 timeline_control
├── services/            # 業務邏輯層
...
- **金鑰與模型**：`GEMINI_API_KEY`、`ELEVENLABS_API_KEY`、`OPENAI_API_KEY`、`MODEL_NAME`（預設 `gemini-3-pro-image-preview`）、`OPENAI_EMBEDDING_MODEL`、`OPENAI_VISION_MODEL`、`OPENAI_TTS_MODEL/VOICE/FORMAT`。可透過 `GENAI_USE_VERTEX` + `VERTEX_PROJECT/LOCATION` 切換 Vertex 端點。
- **資源路徑**：
  ...
  - `NIGHTWALK_ASSETS_DIR`：若存在則掛載為 `/nightwalk_assets` 提供 StaticMode 素材。
...
- **realtime_router (`api/realtime.py`)**
  ...
  - `POST /api/unlock-audio`、`POST /api/remote-click`、`POST /api/video-control`：廣播解鎖音訊、遠端點擊、Video 播放控制等事件。
  - `POST /api/screenshots/request` → `GET /api/screenshots/{id}` → `POST /api/screenshots/{id}/fail`：管理後端排程的截圖任務。
  - `WebSocket /ws/screenshots`：接收 `hello` 後綁定 client id，並推送 screenshot/sound/caption/iframe/collage/remote control/timeline 事件。
- **timeline_router (`api/timeline.py`)**
  - `GET /api/iframe-timelines`、`GET /api/iframe-timelines/{id}`：列出或讀取存於 `metadata/timelines/iframe/*.json` 的時間軸定義。
  - `POST /api/iframe-timelines/{id}/play`：解析 timeline 後透過 `realtime_broadcaster` 廣播 `timeline_control`（含 autoPlay/loop 等選項），由指定 client 執行。
  - `POST /api/iframe-timelines/stop`：通知特定 client 停止播放並可選擇釋放遠端控制權。
...
- **即時事件**：`screenshot_queue` 與 `realtime_broadcaster` 配合 WebSocket `/ws/screenshots`，將 screenshot request/completed、sound_play、subtitle/caption、iframe/collage config、unlock_audio、remote_click、video_control、timeline_control 等事件推送到指定 client，也負責把前端上傳的截圖與狀態寫回 queue。
...
- **Iframe 時間軸（`iframe_timeline.py`）**
  - 統一解析 `metadata/timelines/iframe/*.json`，驗證 timeline id、snapshot 參考與 `target_client_id`。
  - 每個 step 可附帶 `subtitle`、`caption`、`tts`、`remote_clicks`、`video_controls`、`unlock_audio_targets` 等動作；`resolve_iframe_timeline()` 會把這些資訊轉成前端能套用的 payload。
  - 搭配 `timeline_router` 廣播 `timeline_control` 指令，並在停止時釋放 remote config。
- **即時通訊**
  - `realtime_bus.py`：追蹤目前 WebSocket 連線、廣播 sound/iframe/collage/subtitle/caption、unlock_audio、remote_click、video_control、timeline_control 等事件，並允許 target client id。
...
### 7.7 Iframe 時間軸與遠端控制
- Timeline 定義以 JSON 存在 `backend/metadata/timelines/iframe/`，每個檔案描述 snapshot 列表與可選動作（字幕、說明文字、TTS、remote click、video control、unlock audio 等）。
- `/api/iframe-timelines/{id}/play` 會載入定義、執行驗證，並呼叫 `realtime_broadcaster.broadcast_timeline_control`，由前端播放器根據 `options`（autoPlay / startStep / loopOverride / commandId）執行。
- `/api/iframe-timelines/stop` 讓維運側可停止特定 timeline 或釋放遠端控制；必要時可透過 `/api/unlock-audio`、`/api/remote-click`、`/api/video-control` 單發指令，不需經由 timeline。
...
- **時間軸維運**：iframe timeline 仍需手動編輯 JSON，缺少後台 UI 與版本控管，修改後亦須通知前端重新載入。
