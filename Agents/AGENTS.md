...
- **禁止直接操作檔案系統**
- **禁止使用任何非 MCP 工具的功能**
- **禁止用 playwright 導航到後端 API**（`http://localhost:8000` 的端點）
- **禁止嘗試直接訪問後端端點或靜態檔案目錄**

### ✅ 允許事項
- **僅能使用以下兩個 MCP 工具集**：
  1. **glitch_home MCP 工具**（見下方）
  2. **playwright MCP 工具**（瀏覽器自動化，用於截圖、測試、驗證展示效果）
...
### 工具集 2：playwright MCP 工具

playwright MCP 提供瀏覽器自動化功能，可用於：
- **截圖**：擷取前端展示效果
- **導航**：開啟特定 URL 查看展示
- **互動**：點擊、輸入、驗證頁面元素
- **測試**：驗證展示配置是否正確生效
...
- **前端 URL**：`http://localhost:5173`（開發模式）或 `http://localhost:4173`（預覽模式）
- **後端 API**：`http://localhost:8000`（**禁止用 playwright 導航到後端 API**）
- **Agent 專用客戶端 URL**：`http://localhost:5173/?client=agent&iframe_mode=true`（**這是你的預設查看 URL**）
...
4. 使用 `browser_snapshot()` 查看頁面狀態
5. 使用 `browser_take_screenshot()` 截圖記錄效果
...
- ✅ 導航到 `http://localhost:5173/?iframe_mode=true&client=desktop`
- ✅ 導航到 `http://localhost:5173/?img=offspring_xxx.png&client=mobile`
- ✅ 導航到 `http://localhost:5173/?collage_mode=true&client=display_wall`
...
**目標**：在 `desktop` 客戶端建立 2×2 網格，展示 4 張不同圖像

**步驟**：
1. 先查看當前配置：`get_iframe_config("desktop")`
2. 更新配置：
   ```
...
**效果**：`desktop` 客戶端會立即顯示 4 個面板，每個使用不同的視覺化模式。

---

### 場景 2：同步多客戶端展示

**目標**：讓 `desktop` 和 `mobile` 同時顯示相同內容，但佈局不同
...
**目標**：為當前展示的內容生成中文旁白，並在 `desktop` 客戶端顯示字幕
...
**效果**：系統生成音訊檔案，自動播放，並在 `desktop` 客戶端顯示 5 秒字幕。

---

### 場景 4：建立大型拼貼牆

**目標**：在 `display_wall` 客戶端建立 10×10 拼貼牆
...
update_collage_config({
  ...
  "stage_height": 2304,
  "seed": 12345
}, target_client_id="display_wall")
```

**效果**：`display_wall` 客戶端會顯示大型拼貼牆，使用指定的種子值確保可重現。
...
### 2. 客戶端特定 vs 全域配置
- **客戶端特定**：針對特定客戶端（如 `desktop`、`mobile`）設定，適合多螢幕展示
...
- 可讓某些客戶端專注於特定模式（如 `mobile` 使用幻燈片，`desktop` 使用多面板）
...
2. **前端 URL**：前端運行在 `http://localhost:5173`（開發模式），**playwright 必須導航到前端，不是後端**
...
7. **URL 參數**：使用 playwright 導航時，必須包含正確的 URL 參數（如 `iframe_mode=true`、`client=<id>`）
...
2. **選擇工具**：從 glitch_home 或 playwright MCP 工具中選擇合適的工具
3. **執行操作**：使用 MCP 工具完成任務
4. **驗證結果**：使用 playwright 截圖或導航驗證效果
...
**⚠️ 特別注意**：
- 當需要查看展示效果時，**必須導航到前端 URL**（`http://localhost:5173`），而不是後端 API
- **你的專用客戶端 URL**：`http://localhost:5173/?client=agent&iframe_mode=true`（這是預設的查看 URL）
