...
## 前端 Admin Panel
- ScenesManager/ScriptsManager：列表加 version/status/updated_at；可載入指定版；新增草稿播放勾選、發布、回滾按鈕與回滾版本輸入。發布/回滾後會強制載入最新（重置版本輸入）。表單顯示 version/status/published_at/published_by；新增權限提示。
- 草稿播放：播放草稿可勾選 allow draft。
- 權限提示：發布/回滾按鈕標示需 metadata_write，訊息區分草稿/發布/回滾。
- 型別與 API（部分完成）：`types/scene`/`types/script` 增加版本欄位；api client 支援 version/allowDraft 與 versions/publish/rollback；UI 已顯示版本資訊。
