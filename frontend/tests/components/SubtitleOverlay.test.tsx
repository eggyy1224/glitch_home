...
import SubtitleOverlay from "../../src/SubtitleOverlay";
import { createOverlayContent } from "../testUtils";

describe("SubtitleOverlay", () => {
  it("should render subtitle text when subtitle is provided", () => {
    const subtitle = createOverlayContent({
      text: "測試字幕",
      language: "zh-TW",
    });
  ...
  it("should not render when subtitle text is empty", () => {
    const { container } = render(<SubtitleOverlay subtitle={createOverlayContent({ text: "" })} />);
    expect(container.firstChild).toBeNull();
  });
  ...
  it("should not set lang attribute when language is not provided", () => {
    const subtitle = createOverlayContent({ text: "測試字幕", language: null });
